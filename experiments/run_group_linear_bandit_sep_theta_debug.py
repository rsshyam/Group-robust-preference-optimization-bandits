...
from algos.linear_bandit.group_dpo_debug import GroupDirectPolicyOptimizationDebug
from envs.linear_bandit import LinearBandit, ret_feature_func
from envs.group_linear_bandit import GroupLinearBanditSep, GroupLinearBandit, ret_feature_func_debug
...
from utils.collect_data import (
    ...
    collect_group_preference_data_debug,
    collect_rl_data,
    merge_datasets,
    pref_to_rl,
)
...
def main(args):
    ...
    feature_func = ret_feature_func_debug(num_action=action_num, state_dim=state_dim, group_num=group_num)
    ...
    #reward_param = np.array([[1.0/17.0, 16.0/17.0],[16.0/17.0,1.0/17.0]], np.float32)

    #assert group_num == np.shape(reward_param), "The feature is invalid."
    ...
    #Generate datasets:
    pref_data = collect_group_preference_data_debug(args.pref_data_num, env, weights, uniform_policy)
    val_pref = collect_group_preference_data_debug(args.val_data_num, env, val_weights, uniform_policy)
    test_pref = collect_group_preference_data_debug(args.num_trials_for_eval, env, test_weights, uniform_policy)
    #pref_data = collect_preference_data(args.pref_data_num,env,uniform_policy)
    #test_pref = collect_preference_data(args.num_trials_for_eval, env, uniform_policy)
    print(pref_data)
    #opt_reward = env.evaluate_reward(policy=opt_policy)
    opt_reward = env.evaluate_reward_group_wise(policy=opt_policy,states=None)
    ...
    # learn the policy
    policy_feature_func = ret_feature_func_debug(
        num_action=action_num, state_dim=state_dim, group_num=group_num
    )
    agent = GroupDirectPolicyOptimizationDebug(
        state_dim=state_dim,
