...
def parse_args():
    ...
    parser.add_argument("--rdpo_num_iters", type=int, default=200)
    parser.add_argument("--rdpo_adaptive", action="store_true")
    parser.add_argument("--rdpo_ada_coef", type=float, default=1.0)
    parser.add_argument("--rdpo_step_size", type=float, default=0.1)
...
def main(args):
    ...
    logger.info(f"Train a policy solely on the preference data (RDPO).")
    # learn the policy
    ...
    agent = GroupRobustDirectPolicyOptimization(
        ...
        step_size=args.rdpo_step_size,
        num_iters=args.rdpo_num_iters,
        is_adaptive=args.rdpo_adaptive,
        ada_coef=args.rdpo_ada_coef,
    ...
    #reward = agent.train(dataset=pref_data, env=env)####to be edited to include val and test
    # reward = agent.train_by_cvxpy(dataset=pref_data, env=env)
    reward = agent.train(dataset=pref_data, val_dataset=val_pref,
                            test_dataset=test_pref, env=env)
    ...
    logger.info(
        f"Policy parameter learned solely on the preference data (RDPO): {policy_param}."
    )
    logger.info(
        f"Training solely on the preference data (RDPO), dataset size: {len(pref_data): d}, optimal reward: {formatted_opt_reward}, reward: {formatted_reward}, reward error: {formatted_rew_error}."
    )
    rew_err_dict, rew_dict = dict(), dict()
    rew_err_dict[args.pref_data_num] = rew_error
    rew_dict[args.pref_data_num] = reward
    save_path = os.path.join(log_dir, "reward_error_rdpo.yml")
    yaml.dump(rew_err_dict, open(save_path, "w"), default_flow_style=False)
    save_path = os.path.join(log_dir, "reward_rdpo.yml")
