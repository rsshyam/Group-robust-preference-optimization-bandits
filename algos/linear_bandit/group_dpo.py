...
class GroupDirectPolicyOptimization:
    ...
    def train(self, dataset: List[Transition], env: GroupLinearBandit) -> float:
        #print(dataset)
        for step in range(self.num_iters):
            ...
            if step % 20 == 0:
                ...
                if self.logger:
                    self.logger.info(
                        f"Iteration: {step: d}, loss: {loss: .4f}, grad_norm :{grad_norm:.4f}, reward: {formatted_rew}."
                    )
                else:
                    print(
                        f"Iteration: {step: d}, loss: {loss: .4f}, grad_norm :{grad_norm:.4f}, reward: {formatted_rew:}."
    ...
    def train_by_cvxpy(self, dataset: List[Transition], env: GroupLinearBandit) -> float:
    ...
    def evaluate_reward(self, env: GroupLinearBandit) -> float:
        policy = self.ret_policy()
        rew = env.evaluate_reward_group_wise(policy)
