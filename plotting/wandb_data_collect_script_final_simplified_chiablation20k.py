...
SETTINGS = {
    'uneven_imbalanced_ipo': [
        (f'state_dim2action_num8group_num2pref_data_num300weights[0.2,0.8]feature_type{REWARD_FUNC}eval_metricargmax_state-1', 'uneven_imbalanced_ipo'),
        (f'state_dim2action_num8group_num2pref_data_num300weights[0.2,0.8]feature_type{REWARD_FUNC}eval_metricargmax_chiablation', 'chi_ripo'),
    ],
    'uneven_imbalanced_dpo': [
        ...
        (f'state_dim2action_num8group_num2pref_data_num300weights[0.2,0.8]feature_type{REWARD_FUNC}eval_metricargmax_chiablation_isdpo20k', 'IS-DPO'),
        (f'state_dim2action_num8group_num2pref_data_num300weights[0.2,0.8]feature_type{REWARD_FUNC}eval_metricargmax_chiablation_rdpo20k', 'chi_rdpo'),
        ...
        if 'chi' in group[1].lower():
            if 'ripo' in group[1]:
                for chi in list(map(lambda x: x/10, range(0,12,2))):
                    ...
                    if chi==1:
                        group_names.append('$\\textbf{GR-IPO}$' + fr' $\chi={chi}$')
                    elif chi==0:
                        group_names.append('GR-IPO' + fr' $\chi={chi}$' + ' (IS-IPO)')
                    else:
                        group_names.append('GR-IPO' + fr' $\chi={chi}$')
            else:
                for chi in list(map(lambda x: x/10, range(0,12,2))):
                    chi_filter = {**base_filter_dpo, 'group': group[0], 'config.dpo_type': 'rdpo', 'config.chi': chi}
                    filters.append(chi_filter)
                    if chi==1:
                        group_names.append('$\\textbf{GR-DPO}$' + fr' $\chi={chi}$')
                    else:
                        group_names.append('GR-DPO' + fr' $\chi={chi}$')
...
def main(args):
    ...
    for metrics in plot_configs:
        ...
        plot_metric_with_error_bands(iteration_index, values, sems, labels, f'{titles_dict[metric_name]}', subfolder_path, f"{metric_name}", metric=metric_name, extend=True)
